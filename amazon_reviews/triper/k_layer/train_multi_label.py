...
def validate(args, model, type = 'single', test=False):
    ...
    ue           = nn.Parameter(torch.empty(user,12)).cuda()
    ...
    pers_emb = nn.Parameter(torch.empty(6, pro_emb_siz )).cuda()
    nn.init.xavier_uniform_(pers_emb)
    

    zeros_array = np.zeros((user, pro_emb_siz-12))
    ue = torch.hstack((ue, torch.tensor(zeros_array).cuda())).cuda()
    # pdb.set_trace()
    ...
    ei_u2pro     = (torch.tensor(np.load(args.data +'test/ei_u2pro.npy'), dtype=torch.long ).T).cuda()  #y,x fornat
    ...
    ee           = nn.Parameter(torch.empty(ei_pro2u.shape[1],768)).cuda()
    ...
    ei_undirected_user_pro_per = torch.cat((ei_pro2u, ei_u2pro), axis = 1 ).cuda()
    #
    labels = torch.tensor(np.load(args.data + 'test/lab.npy')).cuda()
    ...
    for batch in enumerate(train_loader):
        ...
        users          = batch[1][1][:user].cuda() 
        product        = batch[1][1][user:].cuda()
        
        edge_ind       = batch[1][2][0].cuda()     
        edge_emb_index = batch[1][2][1].cuda()
        ...
        persona_node = persona[1][-6:].cuda()
        persona_neighbours = persona[1][:-6].cuda()   #this has the same order as the ones from user-prod mesage passing
        # pdb.set_trace()
    ...
    # #
        ...
        if type == 'single':
            ...
            y_true.extend(labels_.detach().cpu().numpy())
...
if __name__ == '__main__':
    ...
    model = model = GATPolicy(args.layer).cuda()
    model
    ...
    ue           = nn.Parameter(torch.empty(1000,12)).cuda()
    nn.init.xavier_uniform_(ue)
    ...
    labels = torch.tensor(np.load(args.data + 'train/lab.npy')).cuda()
