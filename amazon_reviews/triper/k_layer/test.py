...
# from model_corr_multi_dummy_cpu_two_L_hack_with_persona_ import *
# from model_corr_multi_dummy_cpu_three_L_hack_with_persona_sayan import *
from model_corr_multi_dummy_cpu_k_layer_generic_hack_with_persona_ import *
...
def validate(args, model, type = 'single', test=False):
    ...
    ue           = torch.tensor(np.load(args.data +'test/ue.npy', allow_pickle=True).astype(np.int32), dtype=torch.float32 ).cuda()
    pe           = torch.tensor(np.load(args.data +'test/pe.npy', allow_pickle=True).astype(np.int32), dtype=torch.float32 ).cuda()
    # ee           = torch.tensor(np.load(args.data +'test/ee.npy'), dtype=torch.float32 ).cuda()

    pers_emb     = torch.tensor(np.load(args.data +'test/pers_emb.npy', allow_pickle=True).astype(np.int32), dtype=torch.float32 ).cuda()
    
    # edgeindex are always in the form of y to x in the (2,n) matrix
    # ei_per2pro   = swap_rows(torch.tensor(np.load(args.data +'test/ei_pro2per_thresh_{args.thresh}.npy', allow_pickle=True).astype(np.int32), dtype=torch.long ).T).cuda()  # y,x fornat
    ei_per2pro   = swap_rows(torch.tensor(np.load(args.data +'test/ei_pro2per.npy', allow_pickle=True).astype(np.int32), dtype=torch.long ).T).cuda()  # y,x fornat
    ei_pro2u     = (torch.tensor(np.load(args.data +'test/ei_pro2u.npy', allow_pickle=True).astype(np.int32), dtype=torch.long ).T).cuda()      # y,x fornat
    ei_u2pro     = (torch.tensor(np.load(args.data +'test/ei_u2pro.npy', allow_pickle=True).astype(np.int32), dtype=torch.long ).T).cuda()      # y,x fornat
    # pdb.set_trace()
    ee           = torch.tensor(np.zeros((ei_pro2u.shape[1], 768)), dtype=torch.float32 ).cuda()
    ...
    zeros_array = np.zeros((ue.shape[0], 1867))
    ...
    ei_undirected_user_pro_per = ei_undirected_user_pro#torch.cat((ei_pro2u, ei_u2pro), axis = 1 )
    #
    ...
    for batch in enumerate(train_loader):
        ...
        # exit(0)   
        # np.save('user.npy', user.detach().cpu().numpy())
        # np.save('per.npy', per.detach().cpu().numpy())
        # print('Saved')
        # exit(0)
        ...
        # to save
        # premuted_labels, predictions
        # np.save('check/predictions.npy', predictions)
        # np.save('check/permuted_labels.npy', predictions)
...
if __name__ == '__main__':
    ...
    # from model_corr_multi_dummy_cpu_two_L_hack import *
    
    model = (GATPolicy(args.layer)).cuda()
