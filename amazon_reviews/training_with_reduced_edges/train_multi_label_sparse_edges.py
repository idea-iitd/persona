...
def validate(args, model, type = 'single', test=False):
    ...
    # ei_per2pro   = swap_rows(torch.tensor(np.load(args.data +'val/edge_pro2per.npy'), dtype=torch.long ).T).cuda() #y,x fornat
    # ei_pro2u     = (torch.tensor(np.load(args.data +'val/edge_pro2u.npy'), dtype=torch.long ).T).cuda()  #y,x fornat
    # ei_u2pro     = (torch.tensor(np.load(args.data +'val/edge_u2pro.npy'), dtype=torch.long ).T).cuda()  #y,x fornat



    # edgeindex are always in the form of y to x in the (2,n) matrix
    ei_per2pro   = swap_rows(torch.tensor(remove_edge( np.load(args.data +'val/edge_pro2per_0_to_2.npy'), args.drop), dtype=torch.long ).T).cuda() #y,x fornat
    # pdb.set_trace()

    ei_pro2u     = remove_edge(np.load(args.data +'val/edge_pro2u.npy'), args.drop)
...
if __name__ == '__main__':
    ...
    # edgeindex are always in the form of y to x in the (2,n) matrix
    # ei_per2pro   = swap_rows(torch.tensor(np.load(args.data +'train/edge_pro2per.npy'), dtype=torch.long ).T).cuda() #y,x fornat
    # ei_pro2u     = (torch.tensor(np.load(args.data +'train/edge_pro2u.npy'), dtype=torch.long ).T).cuda()  #y,x fornat
    # ei_u2pro     = (torch.tensor(np.load(args.data +'train/edge_u2pro.npy'), dtype=torch.long ).T).cuda()  #y,x fornat


    # edgeindex are always in the form of y to x in the (2,n) matrix
    ei_per2pro   = swap_rows(torch.tensor(remove_edge( np.load(args.data +'train/edge_pro2per.npy'), args.drop), dtype=torch.long ).T).cuda() #y,x fornat
    # pdb.set_trace()

    ei_pro2u     = remove_edge(np.load(args.data +'train/edge_pro2u.npy'), args.drop)
    ei_u2pro     = swap_rows(torch.tensor(ei_pro2u))
