...
def validate(args, model, type = 'single', test=False):
    ...
    ei_pro2u     = (torch.tensor(remove_edge(np.load(args.data +'test/ei_pro2u.npy',allow_pickle=True),0.8), dtype=torch.long ).T).cuda()  #y,x fornat
    ei_u2pro     = (torch.tensor(remove_edge(np.load(args.data +'test/ei_u2pro.npy',allow_pickle=True),0.8), dtype=torch.long ).T).cuda()  #y,x fornat
    ...
    for batch in enumerate(train_loader):
        ...
        predictions, user,prod, _ = model(batched_states)  # eval mode
